...
extern "C" {
	NTKERNELAPI NTSTATUS IoCreateDriver(PUNICODE_STRING DriverName,
										PDRIVER_INITIALIZE IntializeFunction);
...
NTSTATUS driver_main(PDRIVER_OBJECT driver_object, PUNICODE_STRING registry_path)
{
	...
	RtlInitUnicodeString(&device_name, L"\\Device\\NeverLockKernel");
	...
	RtlInitUnicodeString(&symbolic_link, L"\\DosDevices\\NeverLockKernel");

	status = IoCreateSymbolicLink(&symbolic_link, &device_name);

	if (status != STATUS_SUCCESS)
	{
		debug_print("Failed to establish symobilic link.\n");
		return status;
	}

	debug_print("Driver symbolic link successfully establised\n");
...
{
	debug_print("Neverlock v0.0.1\n");

	UNICODE_STRING driver_name = {};
	RtlInitUnicodeString(&driver_name, L"\\Driver\\NeverLockKernel");
